...
library(readxl)
deg <- read_excel("/home/gongfengcz/scRNA-heart-mitochodria/data/HCMvsNF.xlsx")
deg1 <- subset(deg,subset = deg$`Cell Type` %in% "Fibroblast")
deg1$change <- ""
...
pathways <- c("cellular respiration","fatty acid catabolic process","fatty acid oxidation","fatty acid metabolic process")
head(circ)
...
p3
ggsave(filename = file.path(output, "心肌整体差异功能富集和弦图.pdf"),
       plot = p3,
       height = 8,
       width = 8)
